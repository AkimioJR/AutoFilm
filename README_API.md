...
一个为 Emby、Jellyfin 服务器提供 Strm 直链播放的小项目，推荐配合 [MediaWarp](https://github.com/AkimioJR/MediaWarp) 使用。

**原作者仓库**: <https://github.com/Akimio521/AutoFilm>

基于原作者功能，新增了 API 调用的方法。希望在 MoviePilot 整理完成或者网盘转存完成后，自动发送 REST 请求去更新 STRM 文件，因此增加了 API 接口。

<br />

推荐配合docker镜像 [kanonhua/cronicle\_tg\_bot](https://hub.docker.com/r/kanonhua/cronicle_tg_bot) 来使用。
...
在 `config/config.yaml` 中增加以下配置：

![配置修改示例](https://github.com/user-attachments/assets/fb39a103-9223-4abc-9b4a-5343f5595bb6)
...
```
http://your_ip:10030/docs
```

![Swagger API 界面](https://github.com/user-attachments/assets/0344749c-1cc1-4ef1-8294-1156a613253c)
...
```bash
curl -X POST "http://localhost:10030/trigger/alist2strm?server_id=动漫" \
...
```bash
curl -X 'POST' \
  'http://your_ip:10030/trigger/alist2strm?server_id=123CLOUD-Movie' \
...
```bash
curl -X POST "http://localhost:10030/trigger/ani2alist?server_id=新番追更" \
  -H "X-API-Key: your_secret_key"
```
...
```bash
curl -X POST "http://localhost:10030/trigger/libraryposter?server_id=我的Jellyfin" \
  -H "X-API-Key: your_secret_key"
```
...
### 场景 2：MoviePilot 整理完成后自动更新

在 MoviePilot 整理完成或网盘转存完成后，通过 API 自动触发 STRM 文件更新：

```bash
# 在 MoviePilot 的自定义脚本中调用
curl -X POST "http://autofilm:10030/trigger/alist2strm?server_id=动漫" \
  -H "X-API-Key: your_secret_key"
```
...
url = "http://localhost:10030/trigger/alist2strm"
params = {"server_id": "动漫"}
...
const response = await fetch(
  'http://localhost:10030/trigger/alist2strm?server_id=动漫',
  {
    method: 'POST',
    headers: {
      'X-API-Key': 'your_secret_key'
...
### Q5: 如何与 MoviePilot 配合使用？

A: 在 MoviePilot 的自定义脚本或插件中，当文件整理完成后调用 AutoFilm 的 API 接口，自动更新 STRM 文件。这样可以实现完全自动化的媒体库管理流程。

## 技术支持

- 原作者项目：<https://github.com/Akimio521/AutoFilm>
- 问题反馈：<https://github.com/AkimioJR/AutoFilm/issues>
- 详细文档：[AutoFilm 说明文档](https://blog.akimio.top/posts/1031/)
- MediaWarp 项目：<https://github.com/AkimioJR/MediaWarp>
